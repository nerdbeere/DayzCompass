...
var DayzSurvivors = function() {
	...
	function getWeaponStats(callback) {
		var items = Helper.arsort(itemCount);
		var newItems = [];
    ...
    function parseHumanity(humanity) {
    ...
    function parseInventory(inventory) {
        return addItems(filterItems(parseValue(inventory)));
    }

    function parseBackpack(backpack) {
        return addItems(filterItems(parseValue(backpack)));
    }
    ...
    function parseSurvivor(survivor) {
        ...
        survivor.humanity = parseHumanity(survivor.humanity);
