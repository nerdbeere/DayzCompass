...
var Survivors = pd.extend({}, Dayz, {
	...
	load: function(callback) {
		if(this.lastUpdate > (+new Date() - 1000)) {
			return callback(this.data);
		}
		var that = this;
		var connection  = new config.Connection().getConnection();

		connection.query('SELECT * FROM survivor INNER JOIN profile ON survivor.unique_id = profile.unique_id WHERE survivor.is_dead = 0 AND survival_time > 0 ORDER BY last_updated DESC', function(err, rows, fields) {
			connection.end();
			if (err) {
				throw err;
			}
	...
	update: function(callback) {
		...
		connection.query('SELECT * FROM survivor INNER JOIN profile ON survivor.unique_id = profile.unique_id WHERE survivor.is_dead = 0 AND survival_time > 0 AND last_updated > NOW() - 180', function(err, rows, fields) {
			connection.end();
			if (err) {
				throw err;
			}
			that.itemCount = {};
			var tmpData = that.parseSurvivors(rows, that);
			that.lastUpdate = +new Date();

			for(var i = 0; i < tmpData.length; i++) {
				if(typeof that.idMap[tmpData[i].unique_id] !== 'undefined') {
					that.data[that.idMap[tmpData[i].unique_id]] = tmpData[i];
				}
			}
