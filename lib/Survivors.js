...
var Survivors = pd.extend({}, Dayz, {
	...
	load: function(callback) {
		...
		var that = this;
		config.connection.query('SELECT * FROM survivor INNER JOIN profile ON survivor.unique_id = profile.unique_id WHERE is_dead = 0 AND survival_time > 0 ORDER BY last_updated DESC', function(err, rows, fields) {
			if (err) {
				throw err;
			}
			that.itemCount = {};
