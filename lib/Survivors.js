...
var Survivors = pd.extend({}, Dayz, {
	...
	load: function(callback) {
		...
		connection.query('SELECT * FROM survivor INNER JOIN profile ON survivor.unique_id = profile.unique_id WHERE survivor.is_dead = 0 AND survival_time > 0 ORDER BY last_updated DESC LIMIT 50', function(err, rows, fields) {
			connection.end();
			if (err) {
				throw err;
			}
